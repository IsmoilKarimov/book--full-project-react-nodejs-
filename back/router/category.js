...
router.get('/', async(req,res)=> {
    ...
    category = category.map((cat,index) =>{
        cat.index = index+1
        cat.status = cat.status == 0 ? '<span class="badge light badge-danger">Nofaol</span>' : '<span class="badge light badge-success">Faol</span>'
        return cat
    })
...
router.put('/save', async(req,res)=>{
    let {_id, title, status, order} = req.body
    status = status || 0
    order = order || 0
